...
import (
	...
	_http "alsritter.icu/middlebaby/internal/proxy/http"
...
func NewServer(r *mux.Router, httpServer *http.Server, imposters []common.HttpImposter) *Server {
	return &Server{
		router:     r,
...
func (s *Server) addImposterHandler() {
	for _, imposter := range s.imposters {
		...
		r := s.router.HandleFunc(url.Path, _http.ImposterHandler(imposter)).
